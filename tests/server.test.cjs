...
test('file API: logical names, compare-and-swap, deletion with text null, whole registry in one request', async t => {
  ...
  assert.deepEqual(registry.layout, { registry: 'TRACKFILE.md', archive: '.trackfile/archive.md', tasks: '.trackfile/tasks', config: '.trackfile/config.json', attachmentsPrefix: '.trackfile/tasks' });
