Trackfile is the task registry of this repository: `TRACKFILE.md` at its data root (milestones, `next_task`, open and recently closed tasks), `.trackfile/archive.md` (closed tasks older than a week, same format) and `.trackfile/tasks/NNN/` (comments in `comments.md` plus attachments). In the layout most repositories use, the data root is the repository root itself. A repository may instead keep this data on a dedicated `trackfile` branch, checked out as a git worktree at `.trackfile/` — recognize this **shared mode** by a `.trackfile.json` file at the repository root (`{"branch": "trackfile", "remote": "origin", "mode": "shared"}`); the rest of this document calls the two "legacy" and "shared". The dashboard runs with `npx trackfile`; the table of contents for agents with `npx trackfile toc` — both resolve the data root for you, from either layout, so nothing below requires knowing which one a given repository uses except the one section below that only applies to shared mode. The rules apply to every coding agent working in this repository (Claude Code, Codex, Cursor, Gemini CLI and others), for code, documentation, tests and configuration alike.

### Shared data branch (only when `.trackfile.json` is present)

- Every registry change goes through a CLI command instead of hand-editing `TRACKFILE.md`/`archive.md`/comments files directly: `trackfile new "title" [--parent ID] [--milestone ID] [--body TEXT]`, `trackfile take NNN`, `trackfile set NNN field value`, `trackfile comment NNN "text"`. Each is one push-or-fail transaction — there is no separate "commit" step and nothing to construct by hand. A manual edit of the worktree at `.trackfile/` is still allowed (a `trackfile toc`/dashboard-style bulk change, say) but must end with `trackfile sync`, which commits, validates and pushes it the same way; a manual edit left unsynced does not exist for anyone else.
- A task's number comes only from `trackfile new` and is final only once the command prints it (after its push succeeded) — do not guess a number from `next_task` yourself, and do not tell the user a number before the command has returned it. Without access to the remote, `trackfile new` refuses outright rather than creating a task only you can see: read and edit existing tasks in that case, but do not create new ones.
- `trackfile take NNN` records the task's number, `assignee` and the branch for you; keep using its output for the `#NNN` in commit subjects and code comments as usual. Before writing `result` or moving a task to `review`, run `trackfile sync` and re-read the task and its comments (`trackfile status` reports right away whether your taken task drifted since — someone may have changed or cancelled it while you worked) — the stale-context check refuses `set`/`comment` outright when it has, explaining what changed; re-read, then retry with `--acknowledge` only once you have accounted for the change.
- The "who changes what" and "statuses, results, commits" rules below apply exactly the same way in shared mode: an agent edits only its own tasks and only ever moves work to `review`, never `done`, without the user's explicit say-so — `trackfile set`/`comment` do not enforce that by themselves.
...
- Before writing, re-read the registry, keep other people's changes and edit only the records you need. In the legacy layout, agents and the dashboard share no transactional lock: never write the same file in parallel; on a conflict, stop and reconcile the current version. Shared mode's CLI commands already serialize writes through the data branch's own lock and push transaction — a rejected push there is retried automatically, not something to reconcile by hand.
