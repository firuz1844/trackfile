...
const { DEFAULTS, readPointer, resolveDataRoot } = require('./layout.cjs');
...
// `dataRoot` differs from `root` only once a project has migrated to shared-branch mode (#205): the
// registry and archive are then created in the data worktree, everything else (gitignore, agent rules,
// launch.json) stays in the source clone since it belongs to that clone, not to the shared data.
function plan({ root, dataRoot = root, name, agentList, global = false, launch = false }) {
  const project = name || path.basename(root);
  const steps = [];
  const exists = p => fs.existsSync(p);
  const registry = path.join(dataRoot, DEFAULTS.registry);
  steps.push(exists(registry) ? { action: 'skip', path: DEFAULTS.registry, reason: 'exists', absolute: registry } : { action: 'create', path: DEFAULTS.registry, content: fill('TRACKFILE.md', project), absolute: registry });
  const archive = path.join(dataRoot, DEFAULTS.archive);
  steps.push(exists(archive) ? { action: 'skip', path: DEFAULTS.archive, reason: 'exists', absolute: archive } : { action: 'create', path: DEFAULTS.archive, content: fill('archive.md', project), absolute: archive });
  // config.json, #206's sync lock and #208's per-agent state are all local, per-machine state — never
  // shared data, whichever layout is in use.
  const LOCAL_STATE = [DEFAULTS.config, '.trackfile/.sync.lock', '.trackfile/.agent/'];
  const gitignore = path.join(root, '.gitignore');
  const ignoreText = exists(gitignore) ? fs.readFileSync(gitignore, 'utf8') : '';
  const ignoredLines = new Set(ignoreText.split(/\r?\n/).map(l => l.trim()));
  const wholeDirIgnored = ignoredLines.has('.trackfile/') || ignoredLines.has('/.trackfile/');
  const missing = wholeDirIgnored ? [] : LOCAL_STATE.filter(p => !ignoredLines.has(p) && !ignoredLines.has('/' + p));
  steps.push(!missing.length ? { action: 'skip', path: '.gitignore', reason: `already ignores ${LOCAL_STATE.join(', ')}` } : { action: exists(gitignore) ? 'append' : 'create', path: '.gitignore', content: (ignoreText && !ignoreText.endsWith('\n') ? ignoreText + '\n' : ignoreText) + missing.join('\n') + '\n' });
  ...
  // #209: a PreToolUse hook on `git commit` warns (stdout only, never blocks) when the agent's own taken
  // task drifted since `take` — the shared-mode stale-context check, surfaced before the agent commits
  // instead of only at its next `set`/`comment`.
  if (agentList.includes('claude')) {
    const file = path.join(root, '.claude', 'settings.json');
    const HOOK_COMMAND = 'npx trackfile hook-precommit';
    let settings = null, broken = false;
    if (exists(file)) { try { settings = JSON.parse(fs.readFileSync(file, 'utf8')); } catch { broken = true; } }
    const preToolUse = Array.isArray(settings?.hooks?.PreToolUse) ? settings.hooks.PreToolUse : [];
    const already = preToolUse.some(entry => entry.matcher === 'Bash' && entry.hooks?.some(h => h.command === HOOK_COMMAND));
    if (broken) steps.push({ action: 'skip', path: '.claude/settings.json', reason: 'is not valid JSON' });
    else if (already) steps.push({ action: 'skip', path: '.claude/settings.json', reason: 'already has the trackfile precommit hook' });
    else {
      const next = settings ?? {};
      next.hooks = { ...next.hooks, PreToolUse: [...preToolUse, { matcher: 'Bash', hooks: [{ type: 'command', command: HOOK_COMMAND }] }] };
      steps.push({ action: settings ? 'append' : 'create', path: '.claude/settings.json', content: JSON.stringify(next, null, 2) + '\n', absolute: file });
    }
  }
...
function run({ cwd = process.cwd(), name, agents: agentList = [], global = false, force = false, dryRun = false, launch = false, log = console.log } = {}) {
  ...
  const { project, steps } = plan({ root, dataRoot, name, agentList: resolveAgents(agentList), global, launch });
  log(`${dryRun ? 'Would initialize' : 'Initializing'} Trackfile in ${root}${dataRoot !== root ? ` (data: ${dataRoot})` : ''} (project "${project}")`);
  let changed = 0;
