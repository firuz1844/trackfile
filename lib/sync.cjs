 ...
 *  - `{ applied: true, pushed: true, commit, result }` once the commit reached the remote — `result` is
 *    whatever `change` itself returned (e.g. `M.addTask`'s `{changes, id}`, so a caller can read `id`).
 *  - `{ applied: true, pushed: false, commit, unpushedCount, result }` when the remote could not be reached at all
...
async function transact(layout, { change, message, mustPush = false, marker = 'agent', maxAttempts = 5, initialDelayMs = 200, log = () => {} }) {
  ...
  async function attempt(attemptNumber, delay) {
    ...
    if (!fetched) return { applied: true, pushed: false, commit: commitSha, unpushedCount: countUnpushed(dataRoot, remote, branch), result: outcome };

    const pushed = git(dataRoot, ['push', `--force-with-lease=refs/heads/${branch}:${remoteBase}`, remote, `refs/heads/${branch}:refs/heads/${branch}`]);
    if (pushed.status === 0) return { applied: true, pushed: true, commit: commitSha, result: outcome };
...
// Shared by `trackfile sync` and the dashboard server (#210): push, and on rejection fall back to an
// actual `git merge` (driven by #207's registered merge driver) rather than reset-and-retry — the caller
// already made a real commit of hand-edited/CAS-written content that has no `change(doc)` function to
// simply re-run, unlike transact()'s own callers. Returns `{pushed:false, unpushedCount}` when the remote
// cannot even be fetched (not a race — nothing to merge against); throws only if a real conflict is left
// after the merge attempt, which needs a human (or, once it's built, #207's `--absorb`).
function pushWithMergeFallback(dataRoot, remote, branch, log = () => {}, validate = () => {}) {
  const fetched = git(dataRoot, ['fetch', '-q', remote, branch]).status === 0;
  if (!fetched) return { pushed: false, unpushedCount: countUnpushed(dataRoot, remote, branch) };
  validate();
  let pushed = git(dataRoot, ['push', remote, `refs/heads/${branch}:refs/heads/${branch}`]);
  if (pushed.status !== 0) {
    log('trackfile: push rejected, merging the remote tip in (using the trackfile-merge driver)...');
    const merge = git(dataRoot, ['merge', '-q', '--no-edit', `${remote}/${branch}`]);
    if (merge.status !== 0) throw new Error(`trackfile: push was rejected and the automatic merge failed — resolve by hand in ${dataRoot}:\n${(merge.stderr || merge.stdout || '').trim()}`);
    // A per-file merge driver cannot check registry/archive references and duplicate IDs together.
    validate();
    pushed = git(dataRoot, ['push', remote, `refs/heads/${branch}:refs/heads/${branch}`]);
    if (pushed.status !== 0) throw new Error(`trackfile: push still failing after merging: ${pushed.stderr.trim()}`);
  }
  return { pushed: true };
}

module.exports = { transact, countUnpushed, isDirty, acquireLock, releaseLock, pushWithMergeFallback, STALE_LOCK_MS };
