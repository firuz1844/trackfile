...
function resolveDataRoot(root, pointer) {
  ...
  if (list !== null) {
    ...
    if (match) return path.resolve(match.path);
  }
  const target = path.join(root, '.trackfile');
  if (fs.existsSync(path.join(target, '.git'))) return target; // already set up; `worktree list` failed transiently
  return createDataWorktree(root, pointer, target);
}
...
module.exports = { DEFAULTS, POINTER, findRoot, readPointer, resolveDataRoot, resolveLayout, relativeOf, diskPath, taskDir, publicLayout, readRegistryFiles };
