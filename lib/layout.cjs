...
// dataBranch lets the reader (#211) recognize the two shared-mode backlink formats
// (`.trackfile/TRACKFILE.md#task-NNN` locally, `blob/<branch>/TRACKFILE.md#task-NNN` on GitHub) without
// hardcoding the branch name "trackfile" — a project may have picked another one.
const publicLayout = layout => ({ registry: layout.registry, archive: layout.archive, tasks: layout.tasks, config: layout.config, attachmentsPrefix: layout.attachmentsPrefix, shared: layout.shared, dataBranch: layout.shared ? layout.pointer.branch : null, dataRemote: layout.shared ? layout.pointer.remote : null });
