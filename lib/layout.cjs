...
function excludeLocalWorktreeState(worktree) {
  ...
  const patterns = ['/.trackfile/config.json', '/.trackfile/.sync.lock'];
...
function resolveDataRoot(root, pointer) {
  ...
  if (list !== null) {
    ...
    if (match) { const resolved = path.resolve(match.path); excludeLocalWorktreeState(resolved); return resolved; }
  }
  const target = path.join(root, '.trackfile');
  if (fs.existsSync(path.join(target, '.git'))) { excludeLocalWorktreeState(target); return target; } // already set up; `worktree list` failed transiently
  const created = createDataWorktree(root, pointer, target);
  excludeLocalWorktreeState(created);
  return created;
}
...
module.exports = { DEFAULTS, POINTER, findRoot, readPointer, resolveDataRoot, resolveLayout, relativeOf, diskPath, taskDir, lockPath, publicLayout, readRegistryFiles };
