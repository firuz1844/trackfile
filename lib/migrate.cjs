...
// The data branch registers its own merge driver (#207) for the two files it actually has structural merge
// logic for; `.trackfile/tasks/**` attachments have no such logic and are left to git's ordinary 3-way
// file merge (or a plain conflict, for a genuine same-path edit — rare enough not to warrant one here).
const GITATTRIBUTES = `${DEFAULTS.registry} merge=trackfile-merge\n${DEFAULTS.archive} merge=trackfile-merge\n`;
...
// touched — safe to run no matter what the agent has checked out or staged right now. `extraBlobs` writes
// synthesized content (not sourced from a file already on disk) alongside the copied data paths.
function createOrphanBranch(root, branch, paths, message, extraBlobs = {}) {
  ...
  const run = (args, opts) => { const r = spawnSync('git', args, { cwd: root, env, encoding: 'utf8', ...opts }); if (r.status !== 0) throw new Error(`git ${args.join(' ')} failed: ${(r.stderr || '').trim()}`); return r.stdout; };
  try {
    run(['read-tree', '--empty']);
    ...
    for (const [posixRel, content] of Object.entries(extraBlobs)) {
      const blob = run(['hash-object', '-w', '--stdin'], { input: content }).trim();
      run(['update-index', '--add', '--cacheinfo', `100644,${blob},${posixRel}`]);
    }
...
function verifyImport(root, paths, branch) {
  ...
  const expectedExtra = 1; // .gitattributes (#207), synthesized rather than copied from a working-tree path
  if (onBranch.size !== posixFiles.length + expectedExtra) throw new Error(`trackfile migrate: verification failed — branch ${branch} has ${onBranch.size} file(s), expected ${posixFiles.length + expectedExtra}.`);
  for (const rel of files) {
    const posixRel = rel.split(path.sep).join('/');
...
async function runMigrate({ root, branch, remote, local, dryRun, task, port, push, skipDashboardCheck, log }) {
  ...
  // #212: the dashboard's own migration wizard calls this with skipDashboardCheck — it IS the dashboard
  // that would otherwise trip this guard on itself, and everything below runs synchronously (spawnSync),
  // so there is no other in-process writer it could actually race against.
  port = port ?? (Number(process.env.PORT) || 3737);
  if (!skipDashboardCheck && await dashboardRunning(port)) throw new Error(`trackfile migrate: something is listening on port ${port} — stop the dashboard for this repository first, it could commit to the old location mid-migration.`);
  ...
  const steps = [
    ...
    ...(push && !local ? [{ name: 'ci', detail: 'push .github/workflows/trackfile-check.yml to the data branch, enable branch protection via gh (best-effort)' }] : []),
  ];
  ...
  const commitSha = createOrphanBranch(root, branch, paths, importMessage, { '.gitattributes': GITATTRIBUTES });
  ...
  const layout = resolveLayout(root); // also makes sure the worktree's local config.json/.sync.lock are excluded (see layout.cjs)
  require('./toc.cjs').toc(layout); // throws if reading from the worktree is somehow broken

  if (push && !local) setupContinuousIntegration({ root, dataRoot: layout.dataRoot, branch, remote, log });
...
// #211: `--push` installs the CI workflow that runs `trackfile check` on every push to the data branch, and
// (best-effort — needs `gh` installed and authenticated against a GitHub repository) turns on branch
// protection requiring it. Neither failure aborts the migration itself, which already succeeded — both are
// logged and left for the user to finish by hand (the README documents the manual steps).
function setupContinuousIntegration({ root, dataRoot, branch, remote, log }) {
  const template = fs.readFileSync(path.join(__dirname, '..', 'templates', 'workflows', 'trackfile-check.yml'), 'utf8').replaceAll('{{BRANCH}}', branch);
  const workflowRel = '.github/workflows/trackfile-check.yml';
  const workflowAbs = path.join(dataRoot, workflowRel);
  fs.mkdirSync(path.dirname(workflowAbs), { recursive: true });
  fs.writeFileSync(workflowAbs, template);
  gitOk(dataRoot, ['add', '-A', '--', workflowRel]);
  gitOk(dataRoot, ['commit', '-q', '-m', '[migrate]: add the trackfile check CI workflow']);
  const pushed = git(dataRoot, ['push', remote, `refs/heads/${branch}:refs/heads/${branch}`]);
  if (pushed.status !== 0) { log(`trackfile migrate: could not push the CI workflow (${pushed.stderr.trim()}) — add and push ${workflowRel} yourself.`); return; }
  log(`trackfile migrate: pushed ${workflowRel}.`);

  if (spawnSync('gh', ['--version']).status !== 0) { log('trackfile migrate: gh CLI not found — set up branch protection for "' + branch + '" yourself (see the README).'); return; }
  const repoView = spawnSync('gh', ['repo', 'view', '--json', 'nameWithOwner'], { cwd: root, encoding: 'utf8' });
  if (repoView.status !== 0) { log('trackfile migrate: `gh repo view` failed (not a GitHub repository, or gh is not authenticated) — set up branch protection yourself.'); return; }
  let nameWithOwner; try { nameWithOwner = JSON.parse(repoView.stdout).nameWithOwner; } catch { log('trackfile migrate: could not parse `gh repo view` output — set up branch protection yourself.'); return; }
  const protection = spawnSync('gh', ['api', '--method', 'PUT', `repos/${nameWithOwner}/branches/${branch}/protection`, '--input', '-'], {
    cwd: root, encoding: 'utf8',
    input: JSON.stringify({ required_status_checks: { strict: true, contexts: ['check'] }, enforce_admins: true, required_pull_request_reviews: null, restrictions: null }),
  });
  if (protection.status !== 0) { log(`trackfile migrate: branch protection setup failed (${(protection.stderr || '').trim()}) — set it up yourself (see the README).`); return; }
  log(`trackfile migrate: branch protection enabled on "${branch}" (requires the "check" status).`);
}
...
async function run({ cwd = process.cwd(), branch = 'trackfile', remote = 'origin', local = false, history = false, push = false, dryRun = false, rollback = false, absorb = null, task = null, port = null, skipDashboardCheck = false, log = console.log } = {}) {
  ...
  return runMigrate({ root, branch, remote, local, dryRun, task, port, push, skipDashboardCheck, log });
}
