...
// #209: the PreToolUse hook on `git commit` calls this — every marker with local agent state (there is
// normally exactly one per machine/worktree, but nothing stops several) gets checked, read-only, no fetch.
function precommitCheck(layout) {
  ...
  for (const file of files) {
    ...
    let state; try { state = JSON.parse(fs.readFileSync(path.join(dir, file), 'utf8')); } catch { continue; }
    const drift = driftOf(doc, state);
    if (drift) drifts.push({ marker: file.slice(0, -'.json'.length), ...drift });
  }
  return drifts;
}

module.exports = { newTask, take, setField, comment, sync, status, precommitCheck, StaleTaskError };
