...
const { relativeOf, diskPath, taskDir, publicLayout, lockPath, resolveLayout, readRegistryFiles } = require('./layout.cjs');
...
const CHANGE_OPERATIONS = new Set(['new task', 'commented', 'edit project', 'edit task', 'edit comment', 'delete comment', 'archive', 'unarchive', 'auto archive']);
...
async function recordChange(req, res, layout) {
  ...
  if (!CHANGE_OPERATIONS.has(operation) || (!taskIds.length && operation !== 'edit project') || taskIds.length > 200 || !taskIds.every(id => typeof id === 'string' && /^\d+$/.test(id)) || !title || title.length > 240 || /[\r\n\0]/.test(title)) {
  ...
  return serialized(layout, M.REGISTRY, async () => {
    let hash, message;
    try {
      message = `${taskIds.length ? taskIds.map(id => '#' + id).join(' ') + ' ' : ''}[${operation}]: ${title}`;
    ...
    try {
      const pushResult = sync.pushWithMergeFallback(repo, layout.pointer.remote, layout.pointer.branch, undefined, () => M.parse(readRegistryFiles(layout)));
