...
const { relativeOf, diskPath, taskDir, publicLayout } = require('./layout.cjs');
...
// Writes to one file are serialized; the expected text is compared right before the rename.
const queues = new Map();
function serialized(name, work) {
  const next = (queues.get(name) || Promise.resolve()).catch(() => {}).then(work);
...
async function recordChange(req, res, layout) {
  ...
  const repo = layout.root, relatives = [...new Set(files)].map(f => relativeOf(layout, f));
  return serialized(M.REGISTRY, async () => {
    try {
      const message = `${taskIds.map(id => '#' + id).join(' ')} [${operation}]: ${title}`;
      ...
      return json(res, 200, { hash, message });
    } catch (error) {
...
async function registry(req, res, layout) {
  ...
  let dirs = [];
  try { dirs = await fs.readdir(path.join(layout.root, layout.tasks)); } catch (e) { if (e.code !== 'ENOENT') throw e; }
  ...
  return json(res, 200, { layout: publicLayout(layout), files });
}
...
async function commitAttachment(repo, relative, message) {
  try {
    ...
    return { hash: (await git(repo, ['rev-parse', 'HEAD'])).trim(), commitError: null };
...
async function attachments(req, res, layout, rest) {
  ...
  const dir = taskDir(layout, taskId), repo = layout.root;
  if (!encodedName) {
    if (req.method !== 'GET') { res.writeHead(405, { allow: 'GET' }); return res.end(); }
    let names;
  ...
  if (req.method === 'PUT') {
    ...
    return serialized('files/' + taskId, async () => {
      ...
      const committed = await commitAttachment(repo, relative, `#${taskId} [attach file]: ${final}`);
  ...
  if (req.method === 'DELETE') {
    return serialized('files/' + taskId, async () => {
      ...
      const committed = tracked ? await commitAttachment(repo, relative, `#${taskId} [delete file]: ${name}`) : { hash: null, commitError: null };
