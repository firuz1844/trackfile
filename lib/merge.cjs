...
function mergeRecord(id, base, ours, theirs) {
  ...
  // Prose has its own three-way history; metadata timestamps must not select its winner.
  let body;
  if (ours.body === theirs.body || theirs.body === base.body) body = ours.body;
  else if (ours.body === base.body) body = theirs.body;
  else throw new MergeConflictError([{ id, field: 'body', ours: ours.body, theirs: theirs.body }]);
  return { data, body };
}
...
function mergeBlocks(type, baseText, oursText, theirsText, { renumber = false } = {}) {
  ...
  // A missing base record is a deletion (including a move to the archive), not an untouched record.
  // Deleting an edited record needs a human: silently choosing either side loses data.
  for (const [id, base] of baseBlocks) {
    const ours = oursBlocks.get(id), theirs = theirsBlocks.get(id);
    if (ours && theirs) continue;
    const survivor = ours || theirs;
    if (!survivor) continue;
    if (!eq(survivor.data, base.data) || survivor.body !== base.body) {
      conflicts.push({ id, field: 'record', ours: ours ? { data: ours.data, body: ours.body } : null, theirs: theirs ? { data: theirs.data, body: theirs.body } : null });
    } else if (theirs) edits.push({ start: theirs.start, end: theirs.end, replacement: '' });
  }

  for (const [id, theirs] of theirsBlocks) {
    const base = baseBlocks.get(id) ?? null;
    const ours = oursBlocks.get(id) ?? null;
    if (!ours || !base) continue; // deletions and additions were handled above
